...
class TestEnergyModel(unittest.TestCase):
    ...
    def test_preprocess_energy_data(self, mock_create_engine, mock_read_sql):
        ...
        dates = pd.date_range('2023-01-01 00:00:00', '2023-01-01 04:00:00', freq='h')
        ...
        self.assertEqual(len(train) + len(test), 5, "Total length should be 5 after resampling")
        ...
        self.assertEqual(interpolated_val, 30.0, "Missing value was not interpolated correctly")
        
        # Assertion 2: Check 80/20 sequential split
        # 5 rows total -> 80% is 4 rows train, 20% is 1 row test
        self.assertEqual(len(train), 4, "Train set should have exactly 4 rows (80% of 5)")
        self.assertEqual(len(test), 1, "Test set should have exactly 1 row (20% of 5)")
