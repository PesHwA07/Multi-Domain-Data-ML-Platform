...
clf_setup = setup(
    ...
    fix_imbalance=True,
    session_id=42,
    verbose=False  # Disable interactive input if running in script
)

# Add PR-AUC as a custom metric so we can sort by it
# PyCaret 3.x uses 'add_metric' function
add_metric('pr_auc', 'PR-AUC', average_precision_score, target='pred_proba')

print("Comparing models based on PR-AUC (this may take a while)...")
# For speed in a quick run, we can exclude some inherently slow models for large datasets or just limit to common tree algorithms, but let's just let it run or exclude SVMs to be safe
best_model = compare_models(sort='PR-AUC', exclude=['svm', 'ridge', 'knn'])
