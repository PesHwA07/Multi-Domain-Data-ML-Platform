...
def ingest_fraud_data():
    ...
    if not os.path.exists(data_path):
        data_path = os.path.join(os.path.dirname(__file__), '../data/raw/fraud/creditcard.csv')
        
    if not os.path.exists(data_path):
        print(f"Dataset not found at {data_path}. Please ensure kagglehub fetched it successfully.")
        return
    ...
    for chunk in pd.read_csv(data_path, chunksize=chunk_size):
        ...
        chunk['timestamp'] = chunk['Time'].apply(lambda x: base_time + timedelta(seconds=x))
        ...
        final_cols = ['transaction_id', 'amount', 'timestamp', 'features', 'is_fraud']
        db_df = chunk[final_cols]
        
        # 5. Insert directly into the Postgres schema
        db_df.to_sql('transactions', schema='fraud', con=engine, if_exists='append', index=False)
